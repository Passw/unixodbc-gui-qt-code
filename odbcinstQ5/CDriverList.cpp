...
#undef DEFLIB_PATH
    ...
    // save changes...
    {
        ...
        {
            ODBCINSTDestructProperties( &hFirstProperty );
            CODBCInst::showErrors( this, QString( tr( "Could not write to (%1)\nYou probably need to do this while logged in as root." ) ).arg( QString::fromLocal8Bit( szINI ) ) );
            return;
        }
...
//    char b2[ 256 ];
    ...
    {
        QMessageBox::warning( this, tr( "ODBC Administrator" ),  tr( "Please select a Driver from the list first." ) );
        return;
    }
    ...
    hFirstProperty->aPromptData     = NULL;
    strncpy( hFirstProperty->szName, "Name", INI_MAX_PROPERTY_NAME );
    strcpy( hFirstProperty->szValue, stringName.toLatin1().data() );
    hLastProperty = hFirstProperty;
    ...
    strncpy( hLastProperty->szName, "Description", INI_MAX_PROPERTY_NAME );
    strcpy( hLastProperty->szValue, "" );
    SQLGetPrivateProfileString( stringName.toLatin1().data(), hLastProperty->szName, "", hLastProperty->szValue, sizeof(hLastProperty->szValue)-1, szINI );
...
*/
    ...
    // NOTE: This is a quick enhancement. Much room to optimize this func.
    {
        ...
        strcpy( szDriverName, stringName.toLatin1().data() );
...
/*                         strncmp( szPropertyNameUpper, "CPREUSE", INI_MAX_PROPERTY_NAME ) != 0 && */
    ...
    if ( pProperties->exec() )
    {
        ...
        if ( SQLWritePrivateProfileString( stringName.toLatin1().data(), NULL, NULL, szINI ) == FALSE )
        {
            delete pProperties;
            ODBCINSTDestructProperties( &hFirstProperty );
            CODBCInst::showErrors( this, QString( tr( "Could not write to (%1).\nYou probably need to do this while logged in as root." ) ).arg( QString::fromLocal8Bit( szINI ) ) );
...
//    HINI        hIni;
    ...
    if ( SQLWritePrivateProfileString( stringName.toLatin1().data(), NULL, NULL, szINI ) == FALSE )
        CODBCInst::showErrors( this, QString( tr( "Could not write property list for (%1)" ) ).arg( stringName ) );
    ...
    if ( iniOpen( &hIni, szINI, (char*)"#;", '[', ']', '=', TRUE ) != INI_ERROR )
    {
        ...
        while ( iniObjectEOL( hIni ) == FALSE )
        {
            ...
            *szSetup          = '\0';
            ...
            ptablewidgetitem = new QTableWidgetItem( QString::fromLocal8Bit( szDriverName ) );
...
//            ptablewidgetitem->setFlags( Qt::ItemIsSelectable );
            ...
            ptablewidgetitem = new QTableWidgetItem( QString::fromLocal8Bit( szSetup ) );
//            ptablewidgetitem->setFlags( Qt::ItemIsSelectable );
        ...
        CODBCInst::showErrors( this, QString( tr( "Could not open system file at %1" ) ).arg( QString::fromLocal8Bit( szINI ) ) );
}
