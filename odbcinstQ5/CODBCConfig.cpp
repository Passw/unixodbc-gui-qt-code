...
void CODBCConfig::loadState()
{
    ...
    int nW = settings.value( QString::fromLocal8Bit( "CODBCConfig/w" ), geometry().width() ).toInt();
    int nH = settings.value( QString::fromLocal8Bit( "CODBCConfig/h" ), geometry().height() ).toInt();
...
void CODBCConfig::saveState()
{
    ...
    settings.setValue( QString::fromLocal8Bit( "CODBCConfig/w" ), width() );
    settings.setValue( QString::fromLocal8Bit( "CODBCConfig/h" ), height() );
}
