...
CDataSourceNamesFileModel::CDataSourceNamesFileModel( QObject *pobjectParent )
    : QDirModel( QStringList( tr( "*.dsn" ) ), QDir::Files, QDir::Name, pobjectParent )
{
}
...
bool CDataSourceNamesFileModel::addDataSourceName( const QString &stringDirectory )
{
    ...
    if ( ODBCINSTConstructProperties( stringDriverName.toLatin1().data(), &hFirstProperty ) != ODBCINST_SUCCESS )
    {
        QMessageBox::warning( 0, tr( "ODBC Administrator" ),  QString( tr( "Could not construct a property list for (%1).\nThis is probably because a viable driver setup library could not be found/used.\nYou may want to try configuring the driver to use a generic setup library." ) ).arg( stringDriverName ) );
        return false;
    }
    ...
    // save properties...
    {
        ...
        QString             stringFileNameAndPath   = QString( tr( "%1/%2.dsn" ) ).arg( stringDirectory ).arg( QString::fromLocal8Bit( hFirstProperty->szValue ) );

        // create the file with a section called "ODBC"...
        if ( !SQLWriteFileDSN( stringFileNameAndPath.toLocal8Bit().data(), "ODBC", NULL, NULL ) )
        {
            CODBCInst::showErrors( 0, QString( tr( "Could not write to (%1)" ) ).arg( stringFileNameAndPath ) );
            ODBCINSTDestructProperties( &hFirstProperty );
            return false;
        }
        ...
        {
            if ( !SQLWriteFileDSN( stringFileNameAndPath.toLatin1().data(), "ODBC", hCurProperty->szName, hCurProperty->szValue ) )
            {
                CODBCInst::showErrors( 0, QString( tr( "Could not write to (%1)" ) ).arg( stringFileNameAndPath ) );
                ODBCINSTDestructProperties( &hFirstProperty );
                return false;
...
bool CDataSourceNamesFileModel::editDataSourceName( const QModelIndex &modelindex )
{
    ...
    szDriver[0] = '\0';
    if ( !SQLReadFileDSN( fileinfo.absoluteFilePath().toLatin1().data(), "ODBC", "DRIVER", szDriver, FILENAME_MAX, NULL ) || strlen( szDriver ) < 1 )
    {
        ...
        szDsn[0] = '\0';
        if ( SQLReadFileDSN( fileinfo.absoluteFilePath().toLatin1().data(), "ODBC", "DSN", szDsn, FILENAME_MAX, NULL ) && strlen( szDsn ) >= 1 )
        {
            UWORD n;
            SQLGetConfigMode( &n ); 
    ...
    {
        CODBCInst::showErrors( 0, QString( tr( "Could not construct a property list for (%1)" ) ).arg( QString::fromLocal8Bit( szDriver ) ) );
        return false;
    }
    ...
    // apply property values from file to properties list...
    {
        ...
        ODBCINSTSetProperty( hFirstProperty, QString( tr( "Name" ) ).toLocal8Bit().data(), fileinfo.baseName().toLocal8Bit().data() );
        ...
        for ( HODBCINSTPROPERTY hCurProperty = hFirstProperty->pNext; hCurProperty != NULL; hCurProperty = hCurProperty->pNext )
        {
            ...
            szAttr[ 0 ] = '\0';
            if ( SQLReadFileDSN( fileinfo.absoluteFilePath().toLocal8Bit().data(), "ODBC", hCurProperty->szName, szAttr, sizeof( szAttr ), NULL ))
            {
    ...
    {
        CPropertiesDialog propertiesdialog( 0, hFirstProperty );
        propertiesdialog.setWindowTitle( tr( "Data Source Properties (edit)" ) );
        if ( !propertiesdialog.exec() )
        {
            ODBCINSTDestructProperties( &hFirstProperty );
            return false;
    ...
    // save changes...
    {
        if ( !SQLWriteFileDSN( fileinfo.absoluteFilePath().toLatin1().data(), "ODBC", NULL, NULL ) )
        {
            CODBCInst::showErrors( 0, QString( tr( "Could not write to (%1)" ) ).arg( fileinfo.absoluteFilePath() ) );
            ODBCINSTDestructProperties( &hFirstProperty );
            return false;
        }
        ...
        for ( HODBCINSTPROPERTY hCurProperty = hFirstProperty->pNext; hCurProperty != NULL; hCurProperty = hCurProperty->pNext )
        {
            if ( !SQLWriteFileDSN( fileinfo.absoluteFilePath().toLatin1().data(), "ODBC", hCurProperty->szName, hCurProperty->szValue ))
            {
                CODBCInst::showErrors( 0, QString( tr( "Could not write to file DSN (%1)" ) ).arg( fileinfo.absoluteFilePath() ) );
                ODBCINSTDestructProperties( &hFirstProperty );
                return false;
            }
        }
...
bool CDataSourceNamesFileModel::deleteDataSourceName( const QModelIndex &modelindex )
{
    ...
    // delete the file...
    if ( unlink( filePath( modelindex ).toLatin1().data() ) )
    {
        QMessageBox::critical( 0, tr( "ODBC Administrator" ), QString( tr( "Unable to unlink %1" ) ).arg( filePath( modelindex ) ) );
        return false;
    }
