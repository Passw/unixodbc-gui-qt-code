	...
	: QWidget( pwidgetParent )
{
    ...
    // create the 'Default' and the 'Current' directory widgets...
    {
        ...
        ptoolbuttonDefault->setIcon( QIcon( QPixmap( xpmSet ) ) );
        ptoolbuttonCurrent->setIcon( QIcon( QPixmap( xpmSet ) ) );
...
{
    if ( SQLWritePrivateProfileString( "ODBC", "FileDSNPath", pFileSelector->getText().toLatin1().constData(), "odbcinst.ini" ) == SQL_FALSE )
    {
...
{
    char szDirectory[FILENAME_MAX + 1];

    szDirectory[0] = '\0';
    _odbcinst_FileINI( szDirectory );

    return QString( QString::fromLocal8Bit( szDirectory ) );
}
