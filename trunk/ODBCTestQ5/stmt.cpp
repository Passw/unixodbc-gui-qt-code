...
void dPrepare::Ok()
{
	...
	sql = str->currentText().toUtf8().constData();
	...
	lname = name_len->currentText().toUtf8().constData();
...
//	SQLTCHAR *	pszSQL 		= NULL;
	...
	{
        stringSQL.clear(); // pszSQL = NULL;
		pOdbcTest->out_win->append( "    Text: <null ptr>" );
	}
	else if ( stringSQL == "<empty string>" || stringSQL.isEmpty() )
	{
		stringSQL = ""; // pszSQL = OtStringToPlatform( s );
		pOdbcTest->out_win->append( "    Text: <empty string>" );
	}
	else
	{
...
/*
        if ( stringSQL.isNull())
            pszSQL = NULL;
		else if ( stringSQL.isEmpty())
			pszSQL = TEXT("");
		else 
			pszSQL = OtStringToPlatform( stringSQL );
*/
		pOdbcTest->out_win->append( "    Text: " + stringSQL ); // pOdbcTest->out_win->append( "    Text: " + OtStringFromPlatform( pszSQL ) );
	}

    /* 
     * Get User provided length...
     */
	QString 	stringLength = name_len->currentText();
     ...
     * Execute SQL...
     */
	SQLRETURN nReturn = SQLExecDirect( hStatement, OtStringToPlatform(stringSQL), nSQL );
...
void dPutData::Ok()
{
	...
	data_str = data_ptr->currentText().toUtf8().constData();
	...
	else if ( strcmp( data_str, "<input window>" ) == 0 )
	{
		...
		if ( s->isEmpty())
			...
			data_ptr_var = (SQLPOINTER) s->toUtf8().constData();
	...
	strlen_or_len_str = str_len->currentText().toUtf8().constData();
