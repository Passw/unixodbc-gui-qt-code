...
void DlgToolsRunAutoTests::Ok()
{
    ...
    *(ServerInfo.szValidServer0)    = '\0';
    ...
    if ( b_log_file->isChecked() ) strcpy( ServerInfo.szLogFile, l_log->text().toUtf8().constData() );
...
void DlgToolsRunAutoTests::runSources( SERVERINFO *pServerInfo )
{
    ...
    foreach( QListWidgetItem *pListWidgetItem, sources->selectedItems() )
    {
        ...
        strcpy( pServerInfo->szSource, stringSource.toUtf8().constData() );

        pOdbcTest->pSettings->beginGroup( stringSource );
        strcpy( pServerInfo->szValidServer0, pOdbcTest->pSettings->value( "SERVER0" ).toString().toUtf8().constData() );
        strcpy( pServerInfo->szValidLogin0, pOdbcTest->pSettings->value( "LOGIN0" ).toString().toUtf8().constData() );
        strcpy( pServerInfo->szValidPassword0, pOdbcTest->pSettings->value( "PASSWORD0" ).toString().toUtf8().constData() );
        strcpy( pServerInfo->szKeywords, pOdbcTest->pSettings->value( "KEYWORDS" ).toString().toUtf8().constData() );
        pOdbcTest->pSettings->endGroup();
